...
let topologicalOSSAElision = FunctionPass(name: "topological-ossa-elision") {
...
private func findMatchingDestroy(for copyInst: CopyValueInst, in block: BasicBlock) -> DestroyValueInst? {
    ...
    while let inst = current {
        ...
        if inst.mayReadFromMemory || inst.mayWriteToMemory {
            // Simplified check since we just need something valid
            return nil
        }
        current = inst.next
    }
